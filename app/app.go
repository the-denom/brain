...
import (
	...
	"github.com/cdbo/brain/x/membership"
	membershipkeeper "github.com/cdbo/brain/x/membership/keeper"
...
func NewWasmApp(
	...
	// CUSTOM WASM ENCODER AND DECODER FOR CUSTOM SDK MODULES
	// encoders := wasmkeeper.DefaultEncoders(appCodec, app.transferKeeper)
	// mergedEncoders := encoders.Merge(CUSTOMMODULE.NewMessageEncoders())
	...
	membershipModule := membership.NewAppModule(
		appCodec,
	...
	app.mm.SetOrderEndBlockers(
		crisistypes.ModuleName,
		membershiptypes.ModuleName, // must come before gov's EndBlock
		govtypes.ModuleName,
		stakingtypes.ModuleName,
		capabilitytypes.ModuleName,
		authtypes.ModuleName,
